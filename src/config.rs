...
"#
    ...
    fn test_config_debug_redacts_token() {
        ...
        assert!(
            debug_output.contains("log_level: \"info\""),
            "Debug output should still show log_level"
        );
        assert!(
            debug_output.contains("https://api.example.com"),
            "Debug output should still show discord_api_url"
        );
    }
    ...
    fn test_args_debug_redacts_token() {
        ...
        assert!(
            debug_output.contains("log_level: Some(\"debug\")"),
            "Debug output should still show log_level"
        );
        assert!(
            debug_output.contains("https://api.example.com"),
    ...
    #[test]
    fn test_args_debug_shows_none_when_token_absent() {
        let args = Args {
            discord_token: None,
            ..Default::default()
        };
        let debug_output = format!("{:?}", args);
        assert!(
            debug_output.contains("discord_token: None"),
            "Debug output should show None when token is not set"
        );
        assert!(
            !debug_output.contains("[REDACTED]"),
            "Debug output should not show [REDACTED] when token is None"
        );
    }
}
